public class ClosedExit : GameGenerator
{
    public override Grid Generate(int playerX)
    {
        ...
        HolePuncher<Tile> puncher = new HolePuncher<Tile>(Tile.empty, Tile.wall).Brush(new SimpleBrush<Tile>(Tile.door));
        ...
        new PathCutter<Tile>(grid.GetTilePosition(Tile.door), new(playerX, grid.Height - 1), t => (int)t).Brush(new SpecificReplacer<Tile>(Tile.empty, [Tile.wall])).Generate(ref grid);
        grid[playerX, grid.Height - 1] = Tile.player;
        ShowMiddleResult(grid);
        grid.PlaceInRadius(Tile.key, new(4, 6), 2, [Tile.empty]);
        new PathCutter<Tile>(grid.GetTilePosition(Tile.key), new(playerX, grid.Height - 1), t => (int)t).Brush(new SpecificReplacer<Tile>(Tile.empty, [Tile.wall])).Generate(ref grid);
        return (Grid)grid;
